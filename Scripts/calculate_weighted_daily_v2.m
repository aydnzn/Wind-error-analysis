...
for day_no=1:day_sz % days
    ...
    sin_wdir_lidar_vectorized = sin(wdir_lidar_vectorized*pi/180);
    cos_wdir_lidar_vectorized = cos(wdir_lidar_vectorized*pi/180);

    wspd_era5_vectorized = reshape(wspd_era5_vec(idx_dataset,:)',[],1);
    wdir_era5_vectorized = reshape(wdir_era5_vec(idx_dataset,:)',[],1);

    sin_wdir_era5_vectorized = sin(wdir_era5_vectorized*pi/180);
    cos_wdir_era5_vectorized = cos(wdir_era5_vectorized*pi/180);
    ...
    wspd_lidar_mean = sum(weight_vec(isfinite(wspd_lidar_vectorized)).*wspd_lidar_vectorized(isfinite(wspd_lidar_vectorized))...
        )/sum(weight_vec(isfinite(wspd_lidar_vectorized)));
    wspd_lidar_mean_vec(day_no) = wspd_lidar_mean;


    wspd_era5_mean = sum(weight_vec(isfinite(wspd_era5_vectorized)).*wspd_era5_vectorized(isfinite(wspd_era5_vectorized))...
        )/sum(weight_vec(isfinite(wspd_era5_vectorized)));
    wspd_era5_mean_vec(day_no) = wspd_era5_mean;


    sin_era5_vectorized = sum(weight_vec(isfinite(sin_wdir_era5_vectorized)).*sin_wdir_era5_vectorized(isfinite(sin_wdir_era5_vectorized))...
        )/sum(weight_vec(isfinite(sin_wdir_era5_vectorized)));
    cos_era5_vectorized = sum(weight_vec(isfinite(cos_wdir_era5_vectorized)).*cos_wdir_era5_vectorized(isfinite(cos_wdir_era5_vectorized))...
        )/sum(weight_vec(isfinite(cos_wdir_era5_vectorized)));


    sin_lidar_vectorized = sum(weight_vec(isfinite(sin_wdir_lidar_vectorized)).*sin_wdir_lidar_vectorized(isfinite(sin_wdir_lidar_vectorized))...
        )/sum(weight_vec(isfinite(sin_wdir_lidar_vectorized)));
    cos_lidar_vectorized = sum(weight_vec(isfinite(cos_wdir_lidar_vectorized)).*cos_wdir_lidar_vectorized(isfinite(cos_wdir_lidar_vectorized))...
        )/sum(weight_vec(isfinite(cos_wdir_lidar_vectorized)));

    degree_era5 = atan2(sin_era5_vectorized,cos_era5_vectorized)*180/pi;
    wdir_era5_mean_vec(day_no)=wrapTo360(degree_era5);

    degree_lidar = atan2(sin_lidar_vectorized,cos_lidar_vectorized)*180/pi;
    wdir_lidar_mean_vec(day_no)=wrapTo360(degree_lidar);

    wspd_lidar_std = sqrt(sum(weight_vec(isfinite(wspd_lidar_vectorized)).*...
        abs(wspd_lidar_vectorized(isfinite(wspd_lidar_vectorized))-wspd_lidar_mean).^2)/sum(weight_vec(isfinite(wspd_lidar_vectorized))));
    wspd_lidar_std_vec(day_no) = wspd_lidar_std;


    wspd_era5_std = sqrt(sum(weight_vec(isfinite(wspd_era5_vectorized)).*...
        abs(wspd_era5_vectorized(isfinite(wspd_era5_vectorized))-wspd_era5_mean).^2)/sum(weight_vec(isfinite(wspd_era5_vectorized))));
    wspd_era5_std_vec(day_no) = wspd_era5_std;

    epsilon_lidar = sqrt(1-(sin_lidar_vectorized.^2 + cos_lidar_vectorized.^2));
    arcsin_lidar = asin(epsilon_lidar)*180/pi;
    wdir_lidar_std = arcsin_lidar*( 1+ ( (2/sqrt(3)-1) * epsilon_lidar.^3 ) );
    wdir_lidar_std=wrapTo360(wdir_lidar_std);
    wdir_lidar_std_vec(day_no) = wdir_lidar_std;

    epsilon_era5 = sqrt(1-(sin_era5_vectorized.^2 + cos_era5_vectorized.^2));
    arcsin_era5 = asin(epsilon_era5)*180/pi;
    wdir_era5_std = arcsin_era5*( 1+ ( (2/sqrt(3)-1) * epsilon_era5.^3 ) );
    wdir_era5_std=wrapTo360(wdir_era5_std);
    wdir_era5_std_vec(day_no) = wdir_era5_std;
...
writetable(My_table,'weighted_daily_wspd_wdir_v3.csv');
